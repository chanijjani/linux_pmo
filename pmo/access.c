		...
		char * key, unsigned char flags)
{
	...
	trace_printk("pmo_stats_start_attachtime_other: mm=%p\n", mm);
	...
	trace_printk("pmo_stats_start_attachtime_wait: mm=%p\n", mm);
	...
	trace_printk("pmo_stats_stop_attachtime_wait: mm=%p\n", mm);
	...
	if (!pmo_bit_is_set(6, pmo) && pmo->state && pmo->state != 1) {
		if (recover (pmo, prot_type, key) == -1) {
			...
			trace_printk("pmo_stats_stop_attachtime_other: mm=%p\n", mm);
...
int do_detach(struct mm_struct *mm, char *path)
{
	...
	trace_printk("pmo_stats_start_detach_time: mm=%p\n", mm);
	...
	trace_printk("pmo_stats_stop_detach_time: mm=%p\n", mm);
