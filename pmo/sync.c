...
void print_hash(char *hash)
{
	printk("%lX %lX %lX %lX %lX %lX %lX %lX %lX %lX %lX %lX %lX %lX %lX"
			...
			hash[22], hash[22], hash[23], hash[24], hash[25], hash[26], hash[27], hash[28], hash[29], hash[30],
			hash[31]);
	return;

}
void pmo_verify_shadow_and_primary(struct vpma_area_struct *vpma, unsigned long int pagenum)
{
	int cmp;
